    ...
    <div className="min-h-screen text-zinc-900 dark:text-white px-4 sm:px-6 lg:px-8 pt-10 pb-24">
      <article className="max-w-3xl mx-auto">
        <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
          <Link href="/blogs" onClick={() => playClick()} className="inline-flex items-center gap-2 text-zinc-500 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-white transition-colors group font-mono text-sm">
        ...
        <motion.footer initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="mt-20 pt-10 border-t border-zinc-900/10 dark:border-white/10">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            ...
            <div className="flex gap-3">
              <Link href="/blogs" onClick={() => playClick()} className="px-6 py-3 rounded-full border border-zinc-900/20 text-zinc-900 hover:bg-zinc-900/5 dark:border-white/15 dark:text-white dark:hover:bg-white/10 text-sm font-semibold transition-all duration-300 hover:scale-105">
                More posts
              </Link>
